...
```
┌─────────────────┐
│  menu_staging   │  ← S3からロード（ステージングテーブル）
└────────┬────────┘
         │
         ▼
┌─────────────────┐
│   ingredient    │  ← Dynamic Table (LAG: 1 minute)
│                 │     成分マスター
└────────┬────────┘
         │
         ▼
┌─────────────────────────────┐
│  ingredient_to_menu_lookup  │  ← Dynamic Table (LAG: 1 minute)
│                             │     成分→メニューマッピング
└────────┬────────────────────┘
         │
         ▼
┌─────────────────────────────┐
│ ingredient_usage_by_truck   │  ← Dynamic Table (LAG: 2 minutes)
│                             │     トラック別成分使用量
└─────────────────────────────┘
```
