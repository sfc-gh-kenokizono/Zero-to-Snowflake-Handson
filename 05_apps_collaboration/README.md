...
| ファイル | 説明 | 使い方 |
|---------|------|--------|
| [`05_apps_collaboration.sql`](./05_apps_collaboration.sql) | **メインスクリプト** | Snowsightで開いて順番に実行 |
| [`05_reset.sql`](./05_reset.sql) | リセット用 | やり直したい時に実行 |
...
1. **Snowsight** にログイン
2. GitHubで [`05_apps_collaboration.sql`](./05_apps_collaboration.sql) を開き、**Raw** → 全文コピー
...
```sql
-- 05_apps_collaboration.sql: 18-20行目
USE DATABASE tb_101;
...
やり直したい場合は [`05_reset.sql`](./05_reset.sql) を実行してください。
